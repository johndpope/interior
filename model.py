    ...
    def create_model(self):
        ...
        with tf.name_scope("real_discriminator"):
            with tf.variable_scope("discriminator"):
                predict_real = self.create_discriminator(self.inputs, self.targets)

        with tf.name_scope("fake_discriminator"):
            with tf.variable_scope("discriminator", reuse=True):
                predict_fake = self.create_discriminator(self.inputs, self.outputs)
        ...
        with tf.name_scope("discriminator_adversarial_train"):
            ...
            discrim_optim = tf.train.AdamOptimizer(self.opt.lr, self.opt.beta1)
        ...
        with tf.name_scope("generator_adversarial_train"):
            with tf.control_dependencies([self.incr_global_step]):
                ...
                gen_optim = tf.train.AdamOptimizer(self.opt.lr, self.opt.beta1)
                gen_grads_and_vars = gen_optim.compute_gradients(self.gen_loss, var_list=gen_tvars)
                self.gen_adversarial_train = gen_optim.apply_gradients(gen_grads_and_vars)
